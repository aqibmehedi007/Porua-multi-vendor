...
class DashboardController extends Controller
{
    ...
    public function getDashboardDetail(Request $request){
        ...
        // free_book
        $free_book = Book::orderBy('book_id', 'desc')->where('price', 0);
        if($request->per_page === 'all'){
            $per_page = $free_book->count();
        }
        $free_book = $free_book->paginate($per_page);
        $free_book = BookResource::collection($free_book);
        
        $top_author                   = AuthorResource::collection(Author::orderBy('author_id','desc')->get());
